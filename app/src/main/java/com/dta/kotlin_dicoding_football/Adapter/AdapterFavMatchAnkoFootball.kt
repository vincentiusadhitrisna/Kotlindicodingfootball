...
class AdapterFavMatchAnkoFootball : RecyclerView.Adapter<AdapterFavMatchAnkoFootball.FootballViewHolder>() {
    private var listItems: List<DataFavorite> = emptyList()

    private var itemClickListener : AdapterFavMatchFootballClickListener? = null

    fun setClickListener(itemClickListener: AdapterFavMatchFootballClickListener){
        this.itemClickListener = itemClickListener
    }
    fun getClickListener(): AdapterFavMatchFootballClickListener?{
        return itemClickListener
    }
    fun setItems(items: List<DataFavorite>?) {
        this.listItems = items!!
        notifyItemRangeInserted(0, items.size)
    }
    fun getItems(): List<DataFavorite>?{
        return this.listItems
    }

    fun getItemByPosition(position: Int): DataFavorite {
